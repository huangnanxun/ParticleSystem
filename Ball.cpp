...
void Win2PPM(int width, int height) {
    ...
    /* Open the file */
    sprintf(fname, "img_ball.png", outdir, counter);
...
float z_position = 0; float z_velocity = 5;
float x_position = 0; float x_velocity = 10;
float y_position = 0; float y_velocity = 8;
float radius = 2; float floorPos = -10; float xleftPos = 10; float xrightPos = -10; float yleftPos = 10; float yrightPos = -10;
float rotate = 0; 


void computePhysics(float dt) {
    float z_acceleration = -10;//-9.8;

    z_velocity = z_velocity + z_acceleration * dt;
    ...
    if (z_position - radius < floorPos) {
        z_position = floorPos + radius;
        z_velocity *= -.85;
    }

    if (x_position - radius < xrightPos) {
        x_position = xrightPos + radius;
        x_velocity *= -.95;
    }else if(x_position + radius > xleftPos){
        x_position = xleftPos - radius;
        x_velocity *= -.95;
    }

    if (y_position - radius < yrightPos) {
        y_position = yrightPos + radius;
        y_velocity *= -.95;
    }
    else if (y_position + radius > yleftPos) {
        y_position = yleftPos - radius;
        y_velocity *= -.95;
    }

    if (z_position == floorPos + radius && abs(z_velocity) < 0.2) {
        x_velocity = 0;
        y_velocity = 0;
    }
    else {
    ...
    printf("rorate: %.2f", rotate);
}

void draw(float dt) {
    glm::mat4 model = glm::mat4();
    model = glm::translate(model, glm::vec3(x_position, y_position, z_position));
    model = glm::rotate(model, glm::radians(rotate), glm::vec3(1.0, 0.0, 1.0));
